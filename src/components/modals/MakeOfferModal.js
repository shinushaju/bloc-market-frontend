...
const MakeOfferModal = ({
  ...
  const inputOffer = (e) => {
    ...
    if (e.target.value > balance) {
      ...
      setMinPriceValidity(
        <p style={{ color: "red" }}>
          Please enter an offer greater than or equal to minimum offer price.
        </p>
      );
      setOffer(e.target.value);
      setDisabled(true);
  ...
  const notificationMessage = (type) => {
    notification[type]({
      description: `You made an offer of ${offer} BLC`,
    });
  };
    ...
    >
      <div className="">
        <div className="my-1" style={{ fontSize: "large" }}>
          Price: <b>{assetData.price} BLC</b>
          <br />
          Minimum Offer Price: <b>{assetData.minPrice} BLC</b>
          <br />
          Highest Offer: <b>{highestOffer}</b>
          {highestOffer !== "No Offers Yet!" && (
            <>
              {" "}
              <b>BLC</b>
            </>
